...
use embassy_sync::{pipe, signal::Signal};
use embassy_sync::pipe::Pipe;

use embassy_futures::select::{select, Either};

// TODO: Revert to SunsetRawMutex since it's good to keep 
// the non-atomic compatibility for, i.e Cortex-M0 processors
...
pub struct TakePipeStorage {
	fanout: Pipe<NoopRawMutex, READ_SIZE>,
    fanin: Pipe<NoopRawMutex, WRITE_SIZE>,
}
...
impl TakePipeStorage {
    ...
    pub fn build(&mut self) -> TakePipe {
        ...
        TakePipe {
            shared_read: SunsetMutex::new((0, fanout_r)),
            shared_write: SunsetMutex::new((0, fanin_w)),
            reader: fanin_r,
            writer: fanout_w,
            wake: Signal::new(),
        }
...
pub struct TakePipe<'a> {
    // fanout
    shared_read: SunsetMutex<(u64, pipe::Reader<'a, NoopRawMutex, READ_SIZE>)>,
    writer: pipe::Writer<'a, NoopRawMutex, READ_SIZE>,
    // fanin
    reader: pipe::Reader<'a, NoopRawMutex, WRITE_SIZE>,
    shared_write: SunsetMutex<(u64, pipe::Writer<'a, NoopRawMutex, WRITE_SIZE>)>,
