...
static RNG_MUTEX: Mutex<CriticalSectionRawMutex, RefCell<Option<&'static mut Rng>>> = Mutex::new(RefCell::new(None));
...
pub fn esp_getrandom_custom_func(buf: &mut [u8]) -> Result<(), getrandom::Error>{
    RNG_MUTEX.lock(|t| {
        ...
        let rng = rng.as_mut().expect("register_custom_rng should have set this");
        rng.read(buf);
    });
    Ok(())
}
