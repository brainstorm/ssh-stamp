...
/// How long `write_frame` waits for the chip to free TX buffer space.
const TX_FREE_TIMEOUT_MS: u32 = 1000;
/// Poll interval while waiting for TX buffer space.
const TX_FREE_POLL_MS: u64 = 2;
...
impl<C: Chip, SPI: SpiDevice> WiznetDevice<C, SPI> {
    ...
    pub async fn read_frame(&mut self, frame: &mut [u8]) -> Result<usize, SPI::Error> {
        ...
        let start_ptr = self.get_rx_read_ptr().await?;
        let mut read_ptr = start_ptr;
    ...
    pub async fn write_frame(&mut self, frame: &[u8]) -> Result<usize, SPI::Error> {
        // Bounded, and yielding between attempts. The original spin had
        // neither: a chip that never reports free space wedges this task
        // forever, and because the loop only awaits SPI it also pins the
        // bus. Dropping a frame is what the caller already tolerates (it
        // discards the result and marks the send done), whereas never
        // returning takes the link down for good.
        let mut waited = 0u32;
        while self.get_tx_free_size().await? < frame.len() as u16 {
            if waited >= TX_FREE_TIMEOUT_MS {
                return Ok(0);
            }
            Timer::after_millis(TX_FREE_POLL_MS).await;
            waited += TX_FREE_POLL_MS as u32;
        }
